...
install_bsdinstaller() {
	...
	sed -i '' -e "s,%%PRODUCT_NAME%%,${PRODUCT_NAME}," \
		  ...
		  -e "s,%%ARCH%%,${TARGET}," \
		  # Let it remain as pfSense.lua because this file contains pfSense-specific overrides to BSDInstaller.lua
	...
	for _pkg_rsync_hostname in ${PKG_RSYNC_HOSTNAME}; do
		...
		if [ -n "${_IS_RELEASE}" -o "${_repo_path_param}" = "${CORE_PKG_PATH}" ]; then
			for _pkg_final_rsync_hostname in ${PKG_FINAL_RSYNC_HOSTNAME}; do
				...
				local _cmd="rsync -Have \"ssh -vvv -i /root/.ssh/id_rsa -p ${PKG_FINAL_RSYNC_SSH_PORT} -o StrictHostKeyChecking=no\" \
