	...
	for _pkg_rsync_hostname in ${PKG_RSYNC_HOSTNAME}; do
		...
		if [ -n "${_IS_RELEASE}" -o "${_repo_path_param}" = "${CORE_PKG_PATH}" ]; then
			for _pkg_final_rsync_hostname in ${PKG_FINAL_RSYNC_HOSTNAME}; do
				...
				local _cmd="rsync -vvv -Have \"ssh -vv -i /root/.ssh/id_rsa -p ${PKG_FINAL_RSYNC_SSH_PORT} -o StrictHostKeyChecking=no\" \
				...
				if script -aq ${_logfile} ssh -vv -p ${PKG_RSYNC_SSH_PORT} -o StrictHostKeyChecking=no \
