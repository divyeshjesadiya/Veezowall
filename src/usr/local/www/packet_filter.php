...
for ($i = 0; $i <= 59; $i+=2) {
  ...
  //print_r($res);
  $json_file = file_get_contents('pfsense_states.json');
  ...
  foreach ($res as $res_key => $res_value) {
    ...
    if(!isset($res_value['src-orig']) || $res_value['src-orig'] == ''){
      unset($res[$res_key]);
    }
    else{
